#!/usr/bin/env python3
"""Generate stiffness profiles using GLOBAL T_K with Sign Alignment.

This script is based on `generate_stiffness_profiles_global_tk.py` but adds a
sign-alignment patch immediately after computing each finger's T_K so that
increasing global EMG activation (mean energy) correlates positively with
projected stiffness axes. This removes arbitrary eigenvector sign ambiguity.

CLI Additions:
    --no-sign-align    Disable sign alignment (for comparison/ablation)
    --verbose          Print detailed diagnostics for each finger axis
"""
...
def _compute_normal_force(df: pd.DataFrame, prefix: Optional[str] = None, n: Tuple[float,float,float] = (0.0,0.0,1.0)) -> np.ndarray:
    ...
    if not np.isfinite(n_norm) or n_norm < 1e-12:
        n_vec = np.array([0.0,0.0,1.0]); n_norm = 1.0
    n_hat = n_vec / n_norm
    Fn = Fc @ n_hat
    return Fn
...
def _ultra_smooth_strong_emg(emg_mag: np.ndarray, fs: Optional[float], cutoff_hz: float = 1.0, window_sec: float = 0.8, sg_window_sec: float = 0.6, sg_poly: int = 3) -> Optional[np.ndarray]:
...
def compute_projection_from_tf(T_F: np.ndarray, reg: float = 1e-6) -> np.ndarray:
    ...
    return 0.5 * (H_F + H_F.T)

def compute_k_basis_from_force_projector(H_F: np.ndarray, target_rank: int = 3, eig_threshold: float = 0.5, near_one_tol: float = 1e-6) -> Tuple[np.ndarray,np.ndarray]:
    dim = H_F.shape[0]
    H_F = 0.5 * (H_F + H_F.T)
    H_K = np.eye(dim) - H_F
    H_K = 0.5 * (H_K + H_K.T)
...
def _plot_variant(time: np.ndarray, forces_signed: np.ndarray, forces_smoothed: np.ndarray, force_normal_abs: Optional[np.ndarray], stiffness_raw: np.ndarray, stiffness_filtered: np.ndarray, emg_raw: np.ndarray, emg_filtered: np.ndarray, out_path: Path, title: str, filter_label: str) -> None:
    if time.ndim != 1 or time.size < 2:
        return
    # Use absolute values of force components for plotting (calculation remains on raw elsewhere)
    f_plot_raw = np.abs(forces_signed)
    f_plot_smooth = np.abs(forces_smoothed)
    fn_plot = np.abs(force_normal_abs) if (force_normal_abs is not None and force_normal_abs.size == time.size) else None
    fig, axes = plt.subplots(3, 1, figsize=(12, 8.0), sharex=True)
    ax_force, ax_stiff, ax_emg = axes
    colors = ["red", "green", "blue"]; axis_labels = ["x", "y", "z"]
    for axis in range(min(3, f_plot_raw.shape[1])):
        color = colors[axis % len(colors)]; label = axis_labels[axis % len(axis_labels)]
        ax_force.plot(time, f_plot_raw[:, axis], color=color, linewidth=0.8, linestyle="--", alpha=0.3, label=f"F_{label}" if axis == 0 else None)
        ax_force.plot(time, f_plot_smooth[:, axis], color=color, linewidth=1.2, label=f"F_{label} smoothed")
    if fn_plot is not None:
        ax_force.plot(time, fn_plot, color="black", linewidth=1.0, linestyle=":", alpha=0.9, label="F_n")
    ...
    ax_emg.set_ylabel("EMG"); ax_emg.set_xlabel("Time [s]"); ax_emg.grid(alpha=0.1); ax_emg.legend(loc="upper right", fontsize=8)
    span = float(time[-1] - time[0]); pad = 0.03 * span if span > 0 else 0.1
    ax_force.set_xlim(float(time[0]) - pad, float(time[-1]) + pad)
    fig.suptitle(f"{title} [{filter_label}]"); fig.tight_layout(rect=(0,0,1,0.97))
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(str(out_path), dpi=160); plt.close(fig)

# ---------------------------------------------------------------------------
# Sign alignment helper
# ---------------------------------------------------------------------------

def _apply_sign_alignment(T_K: np.ndarray, P_global: np.ndarray, finger: str, verbose: bool) -> np.ndarray:
    """Flip rows of T_K so each axis has non-negative correlation with mean EMG energy.

    Args:
        T_K: (r, d) stiffness basis transformation
        P_global: (d, N) global EMG features (smoothed magnitude)
        finger: Finger label for logging
        verbose: Whether to print correlation details
    Returns:
        Possibly sign-flipped T_K (copy)
    """
    if T_K.size == 0 or P_global.size == 0:
        return T_K
    T_mod = T_K.copy()
    emg_mean_energy = np.mean(P_global, axis=0)  # (N,)
    k_projected = T_mod @ P_global               # (r, N)
    for i in range(T_mod.shape[0]):
        corr = np.corrcoef(k_projected[i, :], emg_mean_energy)[0, 1]
        if np.isnan(corr):
            if verbose:
                print(f"[{finger}] Axis {i}: correlation NaN -> keep (skip)")
            continue
        if corr < 0:
            if verbose:
                print(f"[{finger}] Axis {i}: corr={corr:.4f} < 0 -> FLIP")
            T_mod[i, :] *= -1.0
        else:
            if verbose:
                print(f"[{finger}] Axis {i}: corr={corr:.4f} >= 0 -> keep")
    return T_mod

# ---------------------------------------------------------------------------
# Global T_K computation with optional sign alignment
# ---------------------------------------------------------------------------

def compute_global_tk(input_dir: Path, sign_align: bool = True, verbose: bool = False) -> Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
    ...
    # Build global T_K for each finger
    ...
    for finger in ["th", "if", "mf"]:
        ...
        T_K_raw, H_K = compute_k_basis_from_force_projector(H_F, target_rank=3)
        if sign_align:
            T_K = _apply_sign_alignment(T_K_raw, P_global, finger, verbose)
        else:
            T_K = T_K_raw
        global_tk[finger] = (T_K, H_K, T_F, H_F)
        if verbose:
            print(f"[{finger}] T_K norm (post-align): {np.linalg.norm(T_K):.4f}")
    return global_tk
def process_file_with_global_tk(csv_path: Path, output_dir: Path, global_tk: Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]) -> Optional[List[Path]]:
        """Process a single demonstration file using precomputed global T_K per finger."""
        try:
            df = _load_csv(csv_path)
        except Exception as exc:
        ...
        # Deformation & EE positions (fallback to zeros if missing)
        deform_ecc = df["deform_ecc"].to_numpy(dtype=float) if "deform_ecc" in df else np.zeros(len(df), dtype=float)
        ee_if_px = df.get("ee_if_px", df.get("ee_px", pd.Series(np.zeros(len(df))))).to_numpy(dtype=float)
        ee_if_py = df.get("ee_if_py", df.get("ee_py", pd.Series(np.zeros(len(df))))).to_numpy(dtype=float)
        ee_if_pz = df.get("ee_if_pz", df.get("ee_pz", pd.Series(np.zeros(len(df))))).to_numpy(dtype=float)
        ee_mf_px = df.get("ee_mf_px", pd.Series(np.zeros(len(df)))).to_numpy(dtype=float)
        ee_mf_py = df.get("ee_mf_py", pd.Series(np.zeros(len(df)))).to_numpy(dtype=float)
        ee_mf_pz = df.get("ee_mf_pz", pd.Series(np.zeros(len(df)))).to_numpy(dtype=float)
        ee_th_px = df.get("ee_th_px", pd.Series(np.zeros(len(df)))).to_numpy(dtype=float)
        ee_th_py = df.get("ee_th_py", pd.Series(np.zeros(len(df)))).to_numpy(dtype=float)
        ee_th_pz = df.get("ee_th_pz", pd.Series(np.zeros(len(df)))).to_numpy(dtype=float)
        ...
        all_forces_csv: List[np.ndarray] = []
        all_stiffness: List[np.ndarray] = []
        all_stiffness_raw: List[np.ndarray] = []
        all_fn_abs: List[np.ndarray] = []
        out_paths: List[Path] = []
        ...
        for finger, sensor in zip(finger_names, sensor_names):
            ...
            if all(col in df.columns for col in force_cols):
                ...
                all_forces.append(np.zeros((len(time), 3), dtype=float))
                all_forces_csv.append(np.zeros((len(time), 3), dtype=float))
                all_stiffness.append(stiffness_finger)
                all_stiffness_raw.append(stiffness_finger_raw)
                all_fn_abs.append(np.zeros(len(time), dtype=float))

        # Per-finger plots
        for idx, finger_name in enumerate(finger_names):
            out_png = output_dir / f"{csv_path.stem}_{finger_name}_global_tk_signaligned.png"
            try:
                _plot_variant(
                    time,
                    all_forces[idx],
        ...
        # Combined output CSV
        out_csv = output_dir / f"{csv_path.stem}_signaligned.csv"
        try:
            all_forces_combined = np.hstack(all_forces_csv) if all_forces_csv else np.zeros((len(time), 9), dtype=float)
            data: Dict[str, np.ndarray] = {"time_s": time.astype(float)}
        ...
        print(f"[ok] {csv_path.name}: {len(out_paths)} files (GLOBAL T_K SignAligned)")
        return out_paths
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate stiffness with global T_K + sign alignment")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT)
    parser.add_argument("--output-dir", type=Path, default=PKG_DIR / "outputs" / "stiffness_profiles_signaligned")
    parser.add_argument("--validate", action="store_true", help="Save projector validation metrics JSON")
    parser.add_argument("--no-sign-align", action="store_true", help="Disable sign alignment patch")
    parser.add_argument("--verbose", action="store_true", help="Verbose logging for alignment & validation")
    args = parser.parse_args()
    candidates: List[Path] = []
    if isinstance(args.input, Path):
        candidates.append(args.input)
        if not args.input.is_absolute():
            candidates.extend([Path.cwd() / args.input, PKG_DIR / args.input, SCRIPT_DIR / args.input, WORKSPACE_ROOT / args.input])
    for c in candidates:
        if c.is_dir():
            input_dir = c; break
    else:
        raise ValueError(f"Input must be directory: {args.input}")
    global_tk = compute_global_tk(input_dir, sign_align=not args.no_sign_align, verbose=args.verbose)
    if args.validate:
        ...
        for finger, (T_K, H_K, T_F, H_F) in global_tk.items():
            ...
            d = T_K.shape[1]; I = np.eye(d); comp = np.linalg.norm((H_F + H_K) - I) / (np.linalg.norm(I) + EPS)
            report[finger] = {
                ...
                "shapes": {"T_F": list(T_F.shape), "T_K": list(T_K.shape), "H_F": list(H_F.shape), "H_K": list(H_K.shape)},
            }
        import json
        with open(args.output_dir / "global_tk_signaligned_validation.json", "w") as f:
            json.dump(report, f, indent=2)
        if args.verbose:
            print("[validate] Saved validation metrics to", args.output_dir / "global_tk_signaligned_validation.json")
    print("\n[Phase 2] Processing files with global T_K (SignAligned)...")
    ...
    print("\n[Done] All files processed with GLOBAL T_K (SignAligned)")

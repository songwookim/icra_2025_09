#!/usr/bin/env python3
"""Generate stiffness profiles using GLOBAL T_K with ABSOLUTE VALUE of Force.

This version:
1. Computes T_K using ABSOLUTE VALUE of force (|F|) instead of signed force
2. Applies the same T_K to all demos for consistent EMG → Stiffness mapping
"""
...
def _plot_variant(
    ...
    # If signed and smoothed arrays are identical (we pass |F| twice), only plot once (dashed)
    identical = forces_signed.shape == forces_smoothed.shape and np.array_equal(forces_signed, forces_smoothed)
    for axis in range(min(3, forces_signed.shape[1])):
        color = colors[axis % len(colors)]
        label = axis_labels[axis % len(axis_labels)]
        if identical:
            ax_force.plot(time, forces_signed[:, axis], color=color, linewidth=1.2, linestyle="--", label=f"|F_{label}|")
        else:
            ...
            ax_force.plot(time, forces_smoothed[:, axis], color=color, linewidth=1.2, linestyle="--", label=f"|F_{label}| smoothed")
...
def compute_global_tk(input_dir: Path, verbose: bool = False) -> Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
    """Compute global T_K for each finger from all demonstrations combined.
    
    KEY DIFFERENCE: Uses ABSOLUTE VALUE of force |F| instead of signed force.
    ...
    """
    print("\n[Phase 1] Computing global T_K per finger from all demonstrations (FORCE ABS)...")
    ...
    for csv_path in csv_files:
        ...
        try:
            ...
            # Extract full 3D force for each finger and take ABSOLUTE VALUE
            for finger, sensor in sensor_map.items():
                try:
                    ...
                    if all(c in df.columns for c in force_cols):
                        ...
                        # KEY DIFFERENCE: Take absolute value of force
                        forces_abs = np.abs(forces_smoothed)
                        finger_data[finger]["emg"].append(emg_smoothed)
                        finger_data[finger]["force"].append(forces_abs)
                except Exception:
                    continue
    ...
    print(f"[info] Loaded {processed} demonstrations for global T_K (using |F|)")
    ...
    for finger in ["th", "if", "mf"]:
        ...
        F_list = finger_data[finger]["force"]  # each (N_i, 3) - already |F|
...
def process_file_with_global_tk(
    ...
    """Process single file using global T_K per finger (computed from |F|)."""
    try:
        df = _load_csv(csv_path)
    except Exception as exc:
    ...
    for finger, sensor in zip(finger_names, sensor_names):
        # Get this finger's global T_K (computed from |F|)
        ...
        if all(col in df.columns for col in force_cols):
            ...
            # For plotting: use raw forces (signed)
            # For plotting: use absolute (positive) forces
            all_forces.append(np.abs(forces_smoothed))
    ...
    for idx, finger_name in enumerate(finger_names):
        out_png = output_dir / f"{csv_path.stem}_{finger_name}_force_abs.png"
        try:
            _plot_variant(
                time,
                all_forces[idx],
                all_forces[idx],
                all_fn_abs[idx],
                all_stiffness_raw[idx],  # raw
                all_stiffness[idx],  # smoothed
                emg_magnitude,
                emg_smoothed,
                out_png,
                csv_path.stem,
                f"{finger_name} FORCE-ABS",
            )
    ...
    print(f"[ok] {csv_path.name}: {len(out_paths)} files (FORCE ABS)")
    return out_paths


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate stiffness with global T_K using |F| (force absolute value)")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PKG_DIR / "outputs" / "stiffness_profiles_force_abs",
    )
    parser.add_argument("--validate", action="store_true", help="Run geometric/projector validation and save metrics JSON")
    parser.add_argument("--verbose", action="store_true", help="Show per-finger global data & T_K details")
    ...
    # Phase 1: Compute global T_K per finger using |F|
    global_tk = compute_global_tk(input_dir, verbose=args.verbose)
    ...
    if args.validate:
        ...
        import json
        with open(args.output_dir / "global_tk_force_abs_validation.json", "w") as f:
            json.dump(report, f, indent=2)
        print("[validate] Saved validation metrics to", args.output_dir / "global_tk_force_abs_validation.json")
    
    # Phase 2: Process all files with global T_K
    print(f"\n[Phase 2] Processing files with global T_K (|F|)...")
    ...
    print(f"\n[Done] All files processed with GLOBAL T_K using |F|")

...
def save_comparison_figures(sessions: Dict[str, str], gt_df: pd.DataFrame, output_dir: str) -> None:
    ...
    TRIM_SETTINGS = {
        'GT': {'trim_start': 0.05, 'trim_end': 0.95},     # GT: 앞 5%, 뒤 5% 제거
        ...
        for col_idx, finger in enumerate(fingers):
            ...
            for axis_idx, axis in enumerate(['x', 'y', 'z']):
                ...
                if gt_col in gt_df.columns:
                    ...
                    ax.plot(time_norm, vals, '-', linewidth=2.5, color=axis_colors[axis], alpha=0.8, label=axis.upper())
            ax.set_facecolor('#f5f5f5')
            ax.set_title(finger_labels[col_idx], fontsize=15, fontweight='bold')
            if col_idx == 0:
                ax.set_ylabel(row_labels[0], fontsize=16, fontweight='bold')
            ...
            for axis_idx, axis in enumerate(['x', 'y', 'z']):
                ...
                if sess_col in diff_stiff_df.columns:
                    ...
                    vals = smooth_signal(vals_trimmed, window_size=SMOOTH_WIN, method='savgol')
                    time_norm = np.linspace(0, 1, len(vals))
                    ax.plot(time_norm, vals, '-', linewidth=2.5, color=axis_colors[axis], alpha=0.8)
            if col_idx == 0:
                ax.set_ylabel(row_labels[1], fontsize=16, fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.set_xlim(0, 1)
            ...
            for axis_idx, axis in enumerate(['x', 'y', 'z']):
                ...
                if force_col in diff_force_df.columns:
                    ...
                    ax.plot(time_norm, vals, '-', linewidth=2.5, color=axis_colors[axis], alpha=0.8)
            if col_idx == 0:
                ax.set_ylabel(row_labels[2], fontsize=16, fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.set_xlim(0, 1)
            ...
            for axis_idx, axis in enumerate(['x', 'y', 'z']):
                ...
                if force_col in diff_force_df.columns:
                    ...
                    ax.plot(time_norm, vals, '-', linewidth=2.5, color=axis_colors[axis], alpha=0.8)
            if col_idx == 0:
                ax.set_ylabel(row_labels[3], fontsize=16, fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.set_xlim(0, 1)
            ax.set_ylim(force_ylim)  # Diffusion force y축
            ...
            if col_idx == 0:
                ax.set_ylabel(row_labels[4], fontsize=16, fontweight='bold')
            ax.set_xlabel('Time (normalized)', fontsize=13)
            ax.grid(True, alpha=0.3)
            ax.set_xlim(0, 1)
            ax.set_ylim(gt_force_ylim)  # GT force y축
        
        plt.suptitle('GT vs Diffusion Policy: Stiffness & Force Comparison', fontsize=16, fontweight='bold')
            ...
            if model == 'GT':
                ...
                for axis_idx, axis in enumerate(['x', 'y', 'z']):
                    ...
                    if gt_col in gt_df.columns:
                        ...
                        ax.plot(time_norm, vals_smooth, '-', linewidth=2.5, 
                ...
                try:
                    ...
                    for axis_idx, axis in enumerate(['x', 'y', 'z']):
                        ...
                        if sess_col in stiff_df.columns:
                            ...
                            ax.plot(time_norm, vals_plot, '-', linewidth=2.5, 
                                   color=axis_colors[axis], alpha=0.8, label=axis.upper())
                except:
                    pass
    ...
    plt.suptitle('Stiffness per Model (X=Red, Y=Green, Z=Blue)', fontsize=16, fontweight='bold')
    ...
    plt.suptitle('Stiffness per Model - Unified Y-axis (X=Red, Y=Green, Z=Blue)', fontsize=16, fontweight='bold')
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    ...
    plt.suptitle('Stiffness per Model - Sorted by THUMB Pearson (X=Red, Y=Green, Z=Blue)', fontsize=16, fontweight='bold')
    plt.tight_layout(rect=[0, 0, 1, 0.97])
            ...
            if model == 'GT':
                ...
                for axis_idx, axis in enumerate(['x', 'y', 'z']):
                    ...
                    if key in raw_data and raw_data[key]['sess'] is not None:
                        ...
                        ax.plot(time_norm, sess_aligned, '-', linewidth=2.5, 
                               color=axis_colors[axis], alpha=0.8, label=axis.upper())
